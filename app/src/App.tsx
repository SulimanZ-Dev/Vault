...
type VersionComparison={left_version_no:number;right_version_no:number;added_lines:string[];removed_lines:string[];left_sha256:string;right_sha256:string;size_change_bytes:number;page_change:number;date_changed:boolean;identical:boolean}
...
type PluginSummary={id:string;name:string;version:string;description:string;capabilities:string[];data_access:string[];enabled:boolean;approved:boolean;last_run_at:string|null;last_result:string|null}
...
type TestLabScaleResult = {requested:number;total_documents:number;elapsed_ms:number;search_elapsed_ms:number;database_bytes:number}
...
function App() {
  ...
  async function handleSaveTheme(){try{setThemeProfiles(await invoke('save_theme_profile',{name:themeDraft.name,baseMode:themeDraft.base_mode,accent:themeDraft.accent,surfaceMain:themeDraft.surface_main,surfaceSidebar:themeDraft.surface_sidebar,surfaceRaised:themeDraft.surface_raised,textPrimary:themeDraft.text_primary,textSecondary:themeDraft.text_secondary,borderColor:themeDraft.border_color,radiusPx:themeDraft.radius_px,fontScale:themeDraft.font_scale,density:themeDraft.density,motion:themeDraft.motion}));setThemeMessage('Det egna temat sparades')}catch(e){setThemeMessage(String(e))}}
  ...
  return (
    <main className="vault-shell">
      ...
      <section className={`workspace${isDraggingFiles ? ' drag-active' : ''}`} aria-label="Vault arbetsyta" id="vault-workspace" tabIndex={-1}>
          ...
          <section className="operational-view" aria-label="Vault kontrollvy">
            ...
            {activeView === 'security' ? <div className="workflow-stack">
              <div className="work-panel-grid">
                ...
                <div className="work-item"><strong>Windows Hello</strong><span>{windowsHello.available?'Redo för lokal biometrisk verifiering':'Inte tillgängligt'}</span><p>{windowsHello.explanation}</p><small>Verifieringen utförs av Windows. Vault får aldrig fingeravtryck, ansiktsdata eller Windows-PIN.</small></div>
            ...
            {activeView === 'themes' ? <div className="theme-editor-layout">
              ...
              <section className="work-item form-stack theme-form"><strong>Skapa eget tema</strong><input value={themeDraft.name} onChange={e=>setThemeDraft({...themeDraft,name:e.target.value})} placeholder="Temanamn"/><select value={themeDraft.base_mode} onChange={e=>setThemeDraft({...themeDraft,base_mode:e.target.value as 'light'|'dark'})}><option value="dark">Mörk bas</option><option value="light">Ljus bas</option></select><div className="color-field-grid">{([['Accent','accent'],['Bakgrund','surface_main'],['Sidopanel','surface_sidebar'],['Panel','surface_raised'],['Text','text_primary'],['Sekundär text','text_secondary'],['Kantlinje','border_color']] as [string,keyof typeof themeDraft][]).map(([label,key])=><label key={key}>{label}<input type="color" value={String(themeDraft[key])} onChange={e=>setThemeDraft({...themeDraft,[key]:e.target.value})}/></label>)}</div><label>Hörnradie {themeDraft.radius_px}px<input type="range" min="0" max="30" value={themeDraft.radius_px} onChange={e=>setThemeDraft({...themeDraft,radius_px:Number(e.target.value)})}/></label><label>Textstorlek {Math.round(themeDraft.font_scale*100)} %<input type="range" min="0.8" max="1.4" step="0.05" value={themeDraft.font_scale} onChange={e=>setThemeDraft({...themeDraft,font_scale:Number(e.target.value)})}/></label><select value={themeDraft.density} onChange={e=>setThemeDraft({...themeDraft,density:e.target.value as ThemeProfile['density']})}><option value="compact">Kompakt</option><option value="comfortable">Bekväm</option><option value="spacious">Rymlig</option></select><select value={themeDraft.motion} onChange={e=>setThemeDraft({...themeDraft,motion:e.target.value as ThemeProfile['motion']})}><option value="normal">Normala animationer</option><option value="reduced">Minskade animationer</option><option value="off">Animationer av</option></select><button className="primary-action" onClick={handleSaveTheme} type="button">Spara eget tema</button>{themeMessage?<small className="path-value">{themeMessage}</small>:null}</section>
            </div>:null}
        ...
        <div className={['documents', 'inbox', 'archive', 'favorites', 'recent', 'trash'].includes(activeView) ? `content-grid view-${documentViewMode}` : 'content-grid hidden'}>
          ...
          <section className={viewerFullscreen?'preview-pane viewer-fullscreen':'preview-pane'} aria-labelledby="preview-title">
