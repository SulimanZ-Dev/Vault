...
import { NotificationsView, type LocalNotification } from './components/NotificationsView'
import './App.css'
      ...
      <section className={`workspace${isDraggingFiles ? ' drag-active' : ''}`} aria-label="Vault arbetsyta" id="vault-workspace" tabIndex={-1}>
        ...
        {!['documents', 'inbox', 'archive', 'favorites', 'recent', 'trash', 'start'].includes(activeView) ? (
          <section className="operational-view" aria-label="Vault kontrollvy">
            ...
            {activeView==='notifications'?<NotificationsView notifications={localNotifications} onRefresh={()=>void invoke<LocalNotification[]>('list_local_notifications').then(setLocalNotifications)} onUpdate={(id,dismiss)=>void handleNotification(id,dismiss)} onOpenDocument={documentId=>{setSelectedIndex(Math.max(0,productionDocuments.findIndex(document=>document.id===documentId)));setActiveView('documents')}}/>:null}
